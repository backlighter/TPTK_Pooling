...
def store_traj_file(trajs, target_path,Map_target_path, traj_type='raw', extra_fields=None):
    assert traj_type in ['raw', 'mm'], 'only `raw` or `mm` is supported'
...
def store_traj_file_mm_tdrive(trajs, target_path, Map_target_path, traj_type='raw', extra_fields=None):##多线程处理Mapmatching
    print("进入store_traj_file_mm_tdrive")
            ...
            if traj_type == 'raw':
                ...
                for pt in pt_list:
                    ...
                    if candi_pt is not None:
                        ...
                        f_map.write('{},{},{},None,None,None,None,None\n'.format(
                            pt.time.strftime(time_format), pt.lat, pt.lng))

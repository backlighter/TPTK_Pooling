...
def load_rn_shp(path, is_directed=True):
    ...
    for u, v, data in g.edges(data=True):
        ...
        #
        '''
        edge_spatial_idx.insert(int(data['eid']), (env[0], env[2], env[1], env[3]))
        edge_idx[data['eid']] = (u, v)
        '''
        
        '''
        if 'fid' in data and isinstance(data['fid'], list):
            data['eid'] = data['fid'][0]  # 选择列表中的第一个元素作为eid
            edge_spatial_idx.insert(int(data['eid']), (float(env[0]), float(env[2]), float(env[1]), float(env[3])))
            edge_idx[data['eid']] = (u, v)
        '''
        #addcode
        data['eid']=data['fid']

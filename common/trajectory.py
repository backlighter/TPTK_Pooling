...
def parse_traj_file(input_path, traj_type='raw', extra_fields=None):
    ...
    with open(input_path, 'r') as f:
                ...
                if traj_type == 'raw':
                    ...
                    if extra_fields is not None:
                        ...
                        for field in extra_fields:
                            ...
                            field_idx += 1
                    pt = STPoint(lat, lng, datetime.strptime(attrs[1],'%Y-%m-%d %H:%M:%S'), data)
                elif traj_type == 'mm':
                    if attrs[3] == 'None':
                        candi_pt = None
                    else:
        ...
        if len(pt_list) != 0:
            traj = Trajectory(oid, tid, pt_list)
            # print("traj.pt_list为",traj.pt_list)
            trajs.append(traj)
        return trajs
...
def store_traj_file_mm_tdrive(trajs, target_path, Map_target_path, traj_type='raw', extra_fields=None):
    assert traj_type in ['raw', 'mm'], 'only `raw` or `mm` is supported'
    time_format = '%Y/%m/%d %H:%M:%S'
    with open(target_path, 'w') as f_target, open(Map_target_path, 'w') as f_map:
        for traj in trajs:
            pt_list = traj.pt_list
            # print("pt_list为",pt_list)
            # Remove '.csv' from traj.tid
            # tid_without_csv = traj.tid.rstrip('.csv')
            tid_without_csv = traj.tid.replace('.csv', '')
            oid_without_csv = traj.oid.replace('.csv', '')
            # print(tid_without_csv)
            # Writing to Map_target_path
            f_map.write('#,{},{},{},{},{} km\n'.format(tid_without_csv, oid_without_csv, pt_list[0].time.strftime(time_format),
                                                        pt_list[-1].time.strftime(time_format), traj.get_length() / 1000))
            
            if traj_type == 'raw':
                
                for pt in pt_list:
                    f_target.write('{},{},{},{},{}'.format(tid_without_csv,oid_without_csv,pt.time.strftime(time_format), pt.lat, pt.lng))
                    if extra_fields is not None:
                        for extra_field in extra_fields:
                ...
                for pt in pt_list:
                    candi_pt = pt.data['candi_pt']
                    if candi_pt is not None:
                        f_target.write('{},{},{},{},{},{},{},{},{}\n'.format(tid_without_csv, pt.time.strftime(time_format), pt.lat, pt.lng,
                                                                       candi_pt.eid, candi_pt.lat, candi_pt.lng,
                                                                       candi_pt.error, candi_pt.offset))
                    else:
                        f_target.write('{},{},{},{},None,None,None,None,None\n'.format(
                            tid_without_csv, pt.time.strftime(time_format), pt.lat, pt.lng))

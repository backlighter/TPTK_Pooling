...
def process_trajectory(traj, target_path, Map_target_path, traj_type, extra_fields, time_format):
    pt_list = traj.pt_list
    tid_without_last_4 = traj.tid[:-4]

    with open(target_path, 'a') as f_target, open(Map_target_path, 'a') as f_map:
        f_map.write('#,{},{},{},{},{} km\n'.format(tid_without_last_4, traj.oid, pt_list[0].time.strftime(time_format),
                                                    pt_list[-1].time.strftime(time_format), traj.get_length() / 1000))

        if traj_type == 'raw':
            for pt in pt_list:
                f_target.write('{},{},{}'.format(pt.time.strftime(time_format), pt.lat, pt.lng))
                if extra_fields is not None:
                    for extra_field in extra_fields:
                        f_target.write(',{}'.format(pt.data[extra_field]))
                f_target.write('\n')
        elif traj_type == 'mm':
            for pt in pt_list:
                candi_pt = pt.data['candi_pt']
                if candi_pt is not None:
                    f_target.write('{},{},{},{},{},{},{},{},{}\n'.format(tid_without_last_4, pt.time.strftime(time_format), pt.lat, pt.lng,
                                                                    candi_pt.eid, candi_pt.lat, candi_pt.lng,
                                                                    candi_pt.error, candi_pt.offset))
                else:
                    f_target.write('{},{},{},{},None,None,None,None,None\n'.format(
                        tid_without_last_4, pt.time.strftime(time_format), pt.lat, pt.lng))

def store_traj_file_mm_tdrive_parallel(trajs, target_path, Map_target_path, traj_type='raw', extra_fields=None, num_processes=32):
    assert traj_type in ['raw', 'mm'], 'only `raw` or `mm` is supported'

    time_format = '%Y/%m/%d %H:%M:%S'
    
    with Pool(processes=num_processes) as pool:
        pool.starmap(process_trajectory, [(traj, target_path, Map_target_path, traj_type, extra_fields, time_format) for traj in trajs])

# 使用示例
# store_traj_file_mm_tdrive_parallel(trajs, 'target_file.csv', 'map_target_file.csv', traj_type='mm', extra_fields=['field1', 'field2'], num_processes=4)
